/**
 * Sandbox mode — the original simulation debug environment.
 *
 * Extracted from index.ts. Manages the worker lifecycle, event replay,
 * animation loop, and debug UI (Tweakpane + React overlay).
 */
...
import {
  ...
  computeBallData,
  type EventEntry,
  type BallEventSnapshot,
} from './lib/debug/simulation-bridge'
...
export function startSandbox(containerElement: HTMLElement): SandboxInstance {
  ...
  function startSimulation() {
    ...
    if (threeRenderer) {
      if (threeRenderer.domElement.parentElement) {
    ...
    function step(timestamp: number) {
      ...
      for (const circleId of circleIds) {
        ...
        if (nextEvent) {
          ...
          if (config.showCollisions) collisionRenderer.render(circle, progress, nextEvent)
          if (config.showCollisionPreview)
            collisionPreviewRenderer.render(circle, progress, nextEvent, simulatedResults)
          if (config.showFutureTrails) futureTrailRenderer.render(circle, progress, nextEvent, simulatedResults)
        }
      }
