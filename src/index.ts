...
function initScene() {
  ...
  function step(timestamp: number) {
    ...
    if (seekTarget !== null && initialBallStates) {
      ...
      // Rebasing advances the origin to `target` so that ball.position,
      // ball.velocity, and ball.time all reflect the current progress.
      // This is a lossless transformation — the physical trajectory is unchanged.
      for (const id of circleIds) {
        ...
        if (dt > 1e-9) {
          ...
          // trajectory.a (acceleration) is unchanged — same physical trajectory
        }
      }
    } else {
