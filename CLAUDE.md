...
- **RBTree priority queue**: Collisions are stored in a Red-Black Tree sorted by time. `RelationStore` maps circle IDs to their collision entries for efficient cleanup and recomputation.
...
Current coverage: `Circle` class (position, velocity, time advancement) and collision detection (cushion collisions, circle-circle collision times).
